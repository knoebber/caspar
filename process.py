...
def create_dynamo_table():
    return dynamodb.create_table(
        ...
        KeySchema=[
            {
                'AttributeName': DynamoSchema.S3_KEY,
                'KeyType': 'HASH',
            },
            {
                'AttributeName': DynamoSchema.DATA_TS,
                'KeyType': 'RANGE',
            },
...
class DataLabel(str, Enum):
    ...
    def add_zone_to_timestamp(self, ts):
        return f'{ts} -0800'  # they don't update clocks for DST
        ...
        match self:
            ...
            case self.TIMESTAMP:
                ...
                return {
                    'N': str(int(
                        datetime.strptime(
                            self.add_zone_to_timestamp(value),
                            '%m/%d/%Y %I:%M:%S %p %z',
                        ).timestamp()
                    ))
                }
...
def process_obj(s3_key, s3_body, should_save_images_locally=False):
    ...
    dynamo_item_dict = {DynamoSchema.S3_KEY: {'S': s3_key}}
...
def process_all():
    for obj in s3_bucket.objects.all():
        if not obj.key.startswith(CROPS_DIR):
            process_obj(s3_key=obj.key, s3_body=obj.get()['Body'])
